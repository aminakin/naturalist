...
if (!empty($metaTags[$currentURLDir])) {
    ...
    $APPLICATION->SetTitle("Правила проведения розыгрыша призов «Колесо фортуны» - онлайн-сервис бронирования глэмпингов и кемпингов Натуралист");
    $APPLICATION->AddHeadString('<meta name="description" content="Правила проведения розыгрыша призов «Колесо фортуны» | Натуралист - удобный онлайн-сервис поиска и бронирования глэмпинга для отдыха на природе с оплатой на сайте. Вы можете подобрать место для комфортного природного туризма в России по выгодным ценам с моментальной системой бронирования." />');
    $h1SEO = $arSettings['rules_title'];
}
global $arSettings;
?>
