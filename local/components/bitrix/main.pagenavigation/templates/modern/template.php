...
if($arResult['CURRENT_PAGE'] == 1){
	$firstRecord = $arResult["FIRST_RECORD"];
}else{
	$firstRecord = $arResult['PAGE_SIZE'] * ($arResult['CURRENT_PAGE'] - 1);
}

if($arResult['CURRENT_PAGE'] == $arResult['PAGE_COUNT']){
	$lastRecord = $arResult["RECORD_COUNT"];
}else{
	$lastRecord = $arResult["LAST_RECORD"] * $arResult['CURRENT_PAGE'];
}

?>
<? if ($arResult["RECORD_COUNT"] > 0): ?>
	<div class="bx-pagination-total-block"><?= $arParams["TITLE"] . " " . $firstRecord . " &ndash; " . $lastRecord ?><? if ($arParams["SHOW_COUNT"]) echo " " . GetMessage("round_nav_records_of") . " " . $arResult["RECORD_COUNT"]; ?></div>
<? endif; ?>
<div class="modern-page-navigation">
	...
	?>
		...
		do {
			if ($page == $arResult["CURRENT_PAGE"]):
				if ($page == 1):
				?> <span class="modern-page-previous 1"><?= GetMessage("nav_prev") ?></span>
				<? endif; ?>
