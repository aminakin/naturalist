...
//            }
    ...
    {
        // Получаем все разделы с учетом параметров навигации
        $rsSections = CIBlockSection::GetList(
            $this->arSort,
            $this->arFilter,
            false,
            ["IBLOCK_ID", "ID", "NAME", "CODE", "SECTION_PAGE_URL", "UF_*"],
            $this->arNavParams // Передаем параметры навигации
        );

        // Сохраняем все загруженные разделы в массив
        $visibleSectionIds = [];
        while ($arSection = $rsSections->GetNext()) {
            $this->arSections[$arSection['ID']] = $arSection;
            $visibleSectionIds[] = $arSection['ID']; // Сохраняем видимые ID
        }

        // Получаем все ID всех разделов для загрузки цен
        $allSectionsResult = CIBlockSection::GetList(
            [],
            $this->arFilter, // Используйте тот же фильтр, чтобы получить все соответствующие разделы
            false,
            ['ID'] // Получаем только ID всех секций
        );

        $allSectionIds = [];
        while ($arAllSection = $allSectionsResult->GetNext()) {
            $allSectionIds[] = $arAllSection['ID']; // Сохраняем все ID разделов
        }

        // Если есть разделы, получаем цены для всех разделов
        if (!empty($allSectionIds)) {
            $properties = []; // Массив для хранения свойств

            // Запрос на получение свойств (цен)
            $rsProperties = CIBlockElement::GetList(
                [],
                ["SECTION_ID" => $allSectionIds],
                false,
                false,
                ['ID', 'IBLOCK_ID', 'SECTION_ID', 'PROPERTY_CURRENCY', 'PROPERTY_PRICE'] // Замените на необходимые свойства
            );

            while ($arProperty = $rsProperties->GetNext()) {
                // Группируем по ID раздела
                $properties[$arProperty['SECTION_ID']][] = $arProperty;
            }

            // Добавляем цены в массив разделов
            foreach ($this->arSections as &$section) {
                if (isset($properties[$section['ID']])) {
                    $section['PROPERTIES'] = $properties[$section['ID']];
                }
            }
        }

        // Получаем общее количество элементов для постраничной навигации
    ...
    private function fillSections()
    {
        ...
        foreach ($this->arSections as &$arSection) {
            ...
            if ($this->searchedRegionData) {
                ...
                $arSection['DISCTANCE_TO_REGION'] = ucfirst($this->searchedRegionData['UF_CENTER_NAME_RU'] ?? $this->searchedRegionData['CENTER_UF_NAME']);
            } else {
            ...
            if ($this->arExternalInfo) {
                ...
                $sectionPrice = ($arSection["UF_EXTERNAL_SERVICE"] == 1)
                    ? round($arSection['EXTERNAL_DATA']['PRICE'] / $this->daysCount)
            ...
            $uri->addParams(array_merge($this->arUriParams, [
                'sort' => $this->sortBy,
                'order' => $this->sortOrder,
            ]));

            $arSection["URL"] = $uri->getUri();

            $arButtons = CIBlock::GetPanelButtons($arSection["IBLOCK_ID"], $arSection["ID"], 0, ["SECTION_BUTTONS" => false, "SESSID" => false]);
        ...
        if (!empty($arSectionsPrice)) {
            $this->minPrice = min($arSectionsPrice);
            $this->maxPrice = max($arSectionsPrice);
        }
...
$this->arMapSections = $this->arSections;
    ...
    private function fetchPrices()
    {
        ...
        $result = $entity::getList([
            'select' => [
                'ID',
                new ExpressionField('max_price', 'MAX(%s)', 'UF_MIN_PRICE'),
                new ExpressionField('min_price', 'MIN(%s)', 'UF_MIN_PRICE')
            ],
            'filter' => [
                '!UF_MIN_PRICE' => false
            ],
        ...
        foreach ($this->arSections as &$section) {
            if (isset($prices[$section['ID']])) {
                $section['PRICE'] = $prices[$section['ID']]['min_price']; // Или max_price в зависимости от ваших требований
            } else {
                $section['PRICE'] = null;
            }
        }
