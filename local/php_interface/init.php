...
if (!function_exists('initCatalog')) {
  /**
   * Метод для изменения урла
   * при инициализации каталога и карточки товара
   * для установления дат по умолчанию
   */
  function initCatalog()
  {
    $currentUrl = $_SERVER['REQUEST_URI'];
    $isCatalogPage = strpos($currentUrl, '/catalog/') === 0;

    if (!$isCatalogPage) {
      return;
    }

    $hasDateFrom = isset($_GET['dateFrom']) && !empty($_GET['dateFrom']);
    $hasDateTo = isset($_GET['dateTo']) && !empty($_GET['dateTo']);
    $hasGuests = isset($_GET['guests']) && !empty($_GET['guests']);

    if ($hasDateFrom || $hasDateTo || $hasGuests) {
      return;
    }
    ...
    foreach ($_GET as $key => $value) {
      if (!in_array($key, array('dateFrom', 'dateTo', 'guests'))) {
        $newParams[$key] = $value;
      }
    }
    ...
    $newUrl = $path . '?' . http_build_query($newParams);
