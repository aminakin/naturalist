...
class Morpher
{
     ...
     * @throws Exception
     */
    public function __construct($gender = Morpher::GENDER_ANDROGYNOUS)
    {

        static::loadData();

        if (isset($gender) && $gender != Morpher::GENDER_ANDROGYNOUS)
            $this->gender = $gender;
    }
    ...
    public static function detectGender($middlename)
    {
        ...
        switch (mb_substr(mb_strtolower($middlename), -4)) {
            case 'оглы':
                return Morpher::GENDER_MALE;
                break;
            case 'кызы':
                return Morpher::GENDER_FEMALE;
                break;
        }
        ...
        switch (mb_substr(mb_strtolower($middlename), -2)) {
            ...
            case 'на':
                ...
                break;
            default:
                return Morpher::GENDER_ANDROGYNOUS;
                break;
        }
    }

    public function title($title, $case = Morpher::CASE_NOMENATIVE)
    {
        if ($case === Morpher::CASE_NOMENATIVE) {
            return $title;
        }
    ...
    {
        return $this->gender === $this->getGender($gender) || $this->getGender($gender) === Morpher::GENDER_ANDROGYNOUS;
    }
     ...
     * @throws Exception
     */
    public function firstname($firstname, $case = Morpher::CASE_NOMENATIVE)
    {
//		if(empty($firstname))
//			throw new Exception('Firstname cannot be empty.');
     ...
     * @throws Exception
     */
    public function middlename($middlename, $case = Morpher::CASE_NOMENATIVE)
    {
//		if(empty($middlename))
//			throw new Exception('Middlename cannot be empty.');
     ...
     * @throws Exception
     */
    public function lastname($lastname, $case = Morpher::CASE_NOMENATIVE)
    {
//		if(empty($lastname))
